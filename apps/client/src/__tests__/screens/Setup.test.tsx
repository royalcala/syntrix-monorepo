...
describe("Setup", () => {
  ...
  it("renders node ID and join form", () => {
    render(<Wrapped><Setup nodeId="test-node-id" onJoined={() => {}} /></Wrapped>);
  ...
  it("calls join_org on submit and calls onJoined", async () => {
    ...
    render(<Wrapped><Setup nodeId="test-node" onJoined={onJoined} /></Wrapped>);
    const textarea = screen.getByPlaceholderText(/Paste ticket/i);
  ...
  it("shows error when join_org fails", async () => {
    ...
    render(<Wrapped><Setup nodeId="test-node" onJoined={() => {}} /></Wrapped>);
    fireEvent.change(screen.getByPlaceholderText(/Paste ticket/i), { target: { value: "bad-ticket" } });
    fireEvent.click(screen.getByText("Join Organization"));
    await waitFor(() => {
