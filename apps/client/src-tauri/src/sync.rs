...
pub async fn get_sync_info(doc: iroh_docs::api::Doc, store: iroh_blobs::api::Store, node_id: [u8; 32]) -> anyhow::Result<SyncInfo> {
    // 1. Get all registered active members
    let mut member_entries = Box::pin(doc.get_many(iroh_docs::store::Query::key_prefix("members/")).await?);
    ...
    let mut heartbeat_entries = Box::pin(doc.get_many(iroh_docs::store::Query::key_prefix("heartbeat/")).await?);
    ...
    // 3. Combine members and heartbeats
    let now = chrono::Utc::now().timestamp_millis();
    let mut peers = Vec::new();
    let mut processed_peers = std::collections::HashSet::new();

    for (peer_id, member_val) in members {
        let ts = heartbeats.get(&peer_id).cloned().unwrap_or(0);
        let status = if ts > 0 && now - ts < 60000 { "online" } else { "offline" };
        ...
        processed_peers.insert(peer_id.clone());
    ...
    // Add heartbeats for nodes that are not explicitly in members
