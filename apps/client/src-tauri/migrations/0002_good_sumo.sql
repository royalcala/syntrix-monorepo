...
CREATE TABLE `__new_invoice_items` (
	...
	FOREIGN KEY (`invoice_id`) REFERENCES `invoices`(`doc_id`) ON UPDATE no action ON DELETE no action
);
...
CREATE TABLE `__new_order_items` (
	...
	FOREIGN KEY (`order_id`) REFERENCES `orders`(`doc_id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
